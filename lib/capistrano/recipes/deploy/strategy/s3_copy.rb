    ...
    module Strategy
      class S3Copy < Copy
        ...
        def build_aws_install_script
          ...
          configuration[:s3_copy_aws_install_cmd] = "#{aws_environment} s3cmd put #{bucket_name}:#{app_env}/aws_install.sh #{local_output_file} x-amz-server-side-encryption:AES256 2>&1"
        end
